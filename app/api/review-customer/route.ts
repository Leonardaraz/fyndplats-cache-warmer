...
import { getReviewStore, MAX_LIST_ALL, VISIBLE_STATUSES, type StoredReview } from "@/lib/store/reviews";
...
export async function GET() {
  try {
    ...
    for (const status of VISIBLE_STATUSES) {
      const delar = await store.listByStatus(status);
      // ☠️ Ett tak ska synas, aldrig kapa tyst (butikens CLAUDE.md). Listan
      // går nyast först, så egna omdömen — de nyaste — hamnar sist utanför;
      // men äldre egna rader kan ramla bort när importerna växer förbi taket.
      if (delar.length >= MAX_LIST_ALL) kapad = true;
      rader.push(...delar);
    }
    if (kapad) {
      console.error(`[api/review-customer] listByStatus slog i taket (${MAX_LIST_ALL}) — äldre egna omdömen kan saknas. Filtrera på source i frågan.`);
    }
    ...
    return NextResponse.json(
      { ok: true, antal: egna.length, kapad, omdomen: egna },

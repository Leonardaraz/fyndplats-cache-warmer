...
/**
 * Vad Google Shopping-feeden behöver per produkt, i ETT svep över katalogen.
 *
 * Bygger på samma fråga som `listV3ProductPrices` — samma backoff, samma
 * sidpaus, samma tak som KASTAR i stället för att kapa — men med två tunga
 * fält begärda: `MEDIA_ITEMS_INFO` (utan det är galleriet TOMT, uppmätt
 * 2026-08-27) och `PLAIN_DESCRIPTION` (brödtexten kommer inte oombedd).
 *
 * ☠️ INGET SYNLIGHETSVILLKOR I FRÅGAN, av samma skäl som prislistningen:
 * feeden filtrerar på `visible` själv och RÄKNAR utkasten, så en tyst
 * filtrering här hade gjort räknaren blind.
 *
 * Priset går genom `tolkaProduktPris` — en enda definition av "produktens
 * pris", inte en tvilling.
 */
export interface V3FeedProdukt {
  id: string;
  namn: string;
  slug: string;
  visible: boolean;
...
export async function listV3FeedProducts(): Promise<Map<string, V3FeedProdukt>> {
  const ut = new Map<string, V3FeedProdukt>();
  let cursor: string | undefined;
  ...
  for (let sida = 0; sida < MAX_PRIS_SIDOR; sida++) {
    ...
    if (!svar) {
      throw new Error(`V3 feedquery föll på sida ${sida} — ${sistaFel}`);
    }
    const data = (await svar.json()) as {
      products?: Array<{
        id?: string;
        ...
        description?: string;
        media?: {
          main?: { image?: { url?: string }; url?: string };
          itemsInfo?: { items?: Array<{ image?: { url?: string }; url?: string }> };
        };
        actualPriceRange?: { minValue?: { amount?: string }; maxValue?: { amount?: string } };
        variantSummary?: { variantCount?: number };
        inventory?: { availabilityStatus?: string };
    ...
    for (const p of data.products ?? []) {
      ...
      const galleri = (p.media?.itemsInfo?.items ?? [])
        .map((it) => it.image?.url ?? it.url)
        .filter((u): u is string => !!u);
      // Huvudbilden först, resten i galleriordning, utan dubbletter.
      const bilder = Array.from(new Set([...(huvud ? [huvud] : []), ...galleri]));
      ut.set(p.id, {
        id: p.id,
        namn: p.name ?? "",
        slug: p.slug ?? "",
        // Saknat fält räknas som synligt — samma riktning som listVisibleV3ProductIds.
        visible: p.visible !== false,
        beskrivningHtml: p.plainDescription ?? p.description ?? "",
        bilder,
        prisSek: tolkaProduktPris(p).priceSek,
        lagerstatus: p.inventory?.availabilityStatus ?? null,
      });
    }
  ...
  throw new Error(
    `V3 feedquery nådde sidtaket (${MAX_PRIS_SIDOR} sidor, ${ut.size} produkter) med `
      + `markören kvar. Höj taket hellre än att skicka en halv feed till Google.`,
  );
}

# -*- coding: utf-8 -*-
"""Runda 62, Steg 14 — läs de åtta publicerade sidorna som kund.
...
from lint import FRAMMANDE, MEDICINSKT, SUPERLATIV, HUSMARKEN, LANDER  # noqa: E402
...
for p in texter.PRODUKTER:
    ...
    if status != "200":
        rader.append((k, slug, status, age, cache, 0, 0, "—"))
    ...
    for land in LANDSNAMN:
        if re.search(re.escape(land), s, re.I):
    ...
    if i < 0:
        ...
        rader.append((k, slug, status, age, cache, tratt, 0, "—"))
        continue
    if j < i:
    ...
    # Steg 2:s ordagranna förnekelse måste ha nått kunden.
    if "kontorsstol hela arbetsdagen? Nej." not in region:
        fel.append("%s: FAQ:ns förnekelse av kontorsstolsbruk nådde inte kunden" % k)
    ...
    rader.append((k, slug, status, age, cache, tratt, len(region), avvik))

print("%-9s %-26s %-4s %-5s %-8s %-6s %-6s %s"
      % ("id8", "slug", "kod", "age", "cache", "bilder", "text", "mot facit"))
for r in rader:
    print("%-9s %-26s %-4s %-5s %-8s %-6s %-6d %s"
          % (r[0], r[1], r[2], r[3] or "-", r[4] or "-", "%d/5" % r[5], r[6], r[7]))
print()
for f in fel:
    print("FEL:", f)
print()

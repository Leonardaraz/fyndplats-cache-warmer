# -*- coding: utf-8 -*-
"""Runda 110 — bygger JS-anropen för Steg 7 och facit för verifieringen.

☠️ ALLA SEX ÄR UTKAST, och `visible: false` skickas ändå UTTRYCKLIGEN i varje
   PATCH. Runbookens regel är att fältet ska BÄRA ett värde, inte att värdet
   råkar vara rätt: en `variantsInfo`-PATCH utan `visible` PUBLICERAR ett
   utkast (uppmätt 2026-08-28), och en produkt-PATCH som utelämnar fältet är
   samma familj av fel. Värdet härleds ur matt.py i stället för att skrivas
   som konstant, så en blandad batch nästa runda inte kan avpublicera en
   säljande sida — det var runda 109:s fälla.

☠️ KVITTOT ÄR NORMALISERING + EXAKT HASH, inte en längdformel. Wix SKRIVER OM
   markupen vid sparning (<strong> → <span style="font-weight: 700">, <li>text
   → <li><p>text</p>, href → href target="_self"), så det man skickade är inte
   det som lagras. Den SYNLIGA texten är oförändrad — därför räcker inte en
   längdjämförelse, och därför normaliseras facit på samma sätt som Wix gör.
...
☠️ KROPPEN HETER `body`, INTE `data` — uppmätt 2026-09-09 med A/B i samma
   ...
   kropp: en PATCH svarar `revision must not be empty`, men en query svarar
   200 med femtio orelaterade rader.

☠️ HASHEN RÄKNAS MED h*31 % 1e9+7 — exakt i BÅDA språken. FNV-1a:s h*16777619
   överstiger 2^53 i JavaScripts float64 och gav åtta avvikelser av åtta där
   varenda längd stämde: felet låg i hashen, inte i datan.
"""
...
FULLA = {k: v[9] for k, v in matt.RUNDAN.items()}
SYNLIG = {k: False for k in matt.RUNDAN}          # alla sex är utkast
...
if __name__ == "__main__":
  ...
  for (const p of P) {
    ...
    ut.push([p.k,
      ...
      (typeof q.slug === "string" ? q.slug : q.slug && q.slug.name) === p.slug ? "slug ok" : `slug FEL: ${typeof q.slug === "string" ? q.slug : q.slug && q.slug.name}`,
      (tagg("title") || {}).children === p.titel ? "titel ok" : "titel FEL",
      (mt && mt.props.content) === p.meta ? "meta ok" : "meta FEL",
      q.visible === p.synlig ? "utkast ok" : `visible=${q.visible} != ${p.synlig}`,
    ].join(" | "));
  }
  return ut;

# -*- coding: utf-8 -*-
"""Runda 72 — mäter klädselns färg ur PIXLARNA, inte ur feedens Farbe-kolumn.
...
ORDNING = ["64856235","35872574","4f6bef7d","f192540f",
           "78cb09ba","8f6636e4","b8001a1b","dbbe7253"]
KALLA = {"64856235":"Grau","35872574":"Blau","4f6bef7d":"Beige",
         "f192540f":"Schwarz","78cb09ba":"Grau","8f6636e4":"Grau",
         "b8001a1b":"Schwarz","dbbe7253":"Beige+Schwarz"}
# ⚠️ Trion 64856235/35872574/4f6bef7d är GOLVFÅTÖLJER — ryggen fyller nästan
#    hela bilden och sitter LÄGRE än på en vanlig fåtölj. Mätrutan flyttas
#    ned för dem; på en golvfåtölj ligger 12–48 % av höjden delvis i tomrum.
LAG = {"64856235", "35872574", "4f6bef7d"}


def matt(kort):
    im = Image.open(os.path.join(HAR, "rawbilder", "%s_1.jpg" % kort)).convert("RGB")
    w, h = im.size
    # Ryggen ligger i övre mitten på alla åtta produktbilder (kontaktarket visar det).
    if kort in LAG:
        ruta = im.crop((int(w * .20), int(h * .25), int(w * .80), int(h * .60)))
    else:
...
if __name__ == "__main__":
    ...
    for i, k in enumerate(ORDNING):
        im = Image.open(os.path.join(HAR, "rawbilder", "%s_1.jpg" % k)).convert("RGB")
        w, h = im.size
        if k in LAG:
            bit = im.crop((int(w*.20), int(h*.25), int(w*.80), int(h*.60)))
        else:
            bit = im.crop((int(w*.25), int(h*.12), int(w*.75), int(h*.48)))
        bit = bit.resize((CELL, CELL))
        y = PAD + i * (CELL + PAD)
        ark.paste(bit, (PAD, y))

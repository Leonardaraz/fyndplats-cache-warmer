#!/usr/bin/env python3
"""Runda N52: bygger steg 1:s spärrtillägg, steg3.js, steg4.js och steg5.js ur rundans filer.

Samma krav som N31–N37 skrev ned: inget tal och ingen sträng i skrivanropen
får skrivas av för hand. Allt här läses ur ids.tsv, namn.tsv, seo.tsv,
slugs.txt, sku.tsv, variant.tsv, kategori.tsv, vantat-hash.tsv och
media-hash.tsv.

variant.tsv är utläst ur en läsning av rundans produkter i Wix (en GET per
produkt, svaret uttaget ur sessionens logg med skript, aldrig avskrivet) och
kontrolleras mot steg 4:s färska GET i steg 5. Stämplingens variant_skus byggs
ur samma fil (`python3 bygg-steg.py --stampla`), och rutten avvisar ett id som
mappningsraden inte känner (422, ingenting skrivet) — så båda sidorna prövas.
...
if len(sys.argv) > 2 and sys.argv[1] == "--rattelse":
    ...
    ut = ["async function () {",
          "  // Genererad av runda N52:s bygg-steg.py --rattelse ur <kort>.html + raa-hash.tsv — skriv den aldrig för hand.",
          SUMMA_JS,
          "  const PLAN = ["]
    for p in plan:
        ut.append("    {")
...
if len(sys.argv) > 2 and sys.argv[1] == "--seo-rattelse":
    ...
    ut = ["async function () {",
          "  // Genererad av runda N52:s bygg-steg.py --seo-rattelse ur seo.tsv — skriv den aldrig för hand.",
          SUMMA_JS,
          "  const PLAN = ["]
    for r in plan:
...
if len(sys.argv) > 1:
    ...
    if bas.count(ankare) != 1:
        ...
        sys.exit(1)
    tillagg = (
        "  // Tillagd av runda N52:s bygg-steg.py: spärr även över namn, slug och SEO,\n"

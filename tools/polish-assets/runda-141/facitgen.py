...
Aritmetiken (h*31 % 1e9+7) är exakt i BÅDE Python och JavaScript.
"""
import json
import os
import re
...
if __name__ == "__main__":
    ...
    for pid in T.SLUG:
        ...
        facit[pid] = {
            ...
            "slug": T.SLUG[pid],
            "sku": T.SKU[pid],
            "wixVariantId": T.WIX_VARIANT[pid],
            "seoTitel": T.TITEL[pid],
            "seoBeskrivning": T.META[pid],
            "kalla_tecken": len(h),
    ...
    for pid, d in facit.items():
        print(f"{pid}  källa {d['kalla_tecken']:5d} -> lagrat "
              f"{d['lagrat_tecken']:5d}  synlig {d['synlig_tecken']:5d}  "
              f"hash {d['synlig_hash']}  {d['sku']}")

# -*- coding: utf-8 -*-
"""Runda 79, Steg 14 — läs de åtta publicerade sidorna som kund.
...
början. Kontrollmätt mot rundans egen text får dessa INTE stå med:
  ...
  ord        varför
  ---------  ------------------------------------------
  rollen     "Rollen" ÄR svenska (rollens, rollen)
  rader      svenskt ord — och står i källans tyska text
  hocker     "hockey"; dessutom nära vårt eget "hocker"-fria språk
  gas        vår egen text säger "gaslyft" ÖVERALLT
  arm        vår text säger "armstöd"
  matt / samt / beige / dunkel / hell(re) / grun(d) / polster
             rena svenska ord eller svenska ordinledningar
  ring       vår text säger "ringrygg", "ringen"
  stoff      svenskt "stoff"

Sammansättningarna är säkra: `rollhocker`, `werkstatthocker`, `armlehne`,
`hellgrau`, `dunkelgrau`. `ergonomisch` är säkert trots svenskans
*ergonomisk* — mönstret kräver ett `c` där svenskan har ett `k`.
...
TYSKA_RULLPALL = [
    "rollhocker", "werkstatthocker", "pendelhocker", "arbeitshocker",
    "sitzhocker", "drehhocker", "sattelhocker", "salonhocker", "kniehocker",
    "arbeitsstuhl", "drehstuhl", "bürostuhl", "burostuhl", "computerstuhl",
    "rollsitz", "rollsessel", "höhenverstellbar", "hohenverstellbar",
    "höhenverstellung", "sitzfläche", "sitzflache", "sitzhöhe", "sitzhohe",
    "rückenlehne", "ruckenlehne", "armlehne", "lendenstütze", "lendenstutze",
    "fußstütze", "fussstutze", "fußkreuz", "gaslift", "gasfeder",
    "kunstleder", "leinenoptik", "schaumstoff", "bremsrollen",
    "gummilaufrollen", "ablagefläche", "ablageflache", "schublade",
    "werkzeugablage", "tragfähigkeit", "tragfahigkeit", "belastbar",
    "ergonomisch", "atmungsaktiv", "wippfunktion", "drehbar", "verstellbar",
    "hellgrau", "dunkelgrau", "grau", "braun", "weiß", "weiss", "grün",
    "schwarz", "blau",
]
...
for p in texter.PRODUKTER:
    ...
    for o in TYSKA + TYSKA_RULLPALL + HUSMARKEN + ATTRIBUTION:

#!/usr/bin/env python3
"""Runda N35: bygger steg 1:s spärrtillägg, steg3.js, steg4.js och steg5.js ur rundans filer.

Samma krav som N31–N34 skrev ned: inget tal och ingen sträng i skrivanropen
...
läste alla åtta plus reserven 093aedd2, alla `ref: main`) och kontrolleras
...
if len(sys.argv) > 2 and sys.argv[1] == "--rattelse":
    ...
    ut = ["async function () {",
          "  // Genererad av runda N35:s bygg-steg.py --rattelse ur <kort>.html + raa-hash.tsv — skriv den aldrig för hand.",
          SUMMA_JS,
          "  const PLAN = ["]
    for p in plan:
        ut.append("    {")
...
if len(sys.argv) > 1:
    ...
    if bas.count(ankare) != 1:
        ...
        sys.exit(1)
    tillagg = (
        "  // Tillagd av runda N35:s bygg-steg.py: spärr även över namn, slug och SEO,\n"

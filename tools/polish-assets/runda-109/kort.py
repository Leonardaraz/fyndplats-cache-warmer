# -*- coding: utf-8 -*-
"""Runda 109 — kort för familjens två nya färger (Leonards regel 2026-08-26).

Bara rundans TVÅ får kort här. Runda 108:s sex har sina kort i Wix sedan i går,
och deras spec-tabell är oförändrad — Steg 7 rörde två stycken brödtext, inte
en enda rad i tabellen. Att bygga om dem hade gett sex nya mediafiler utan en
enda skillnad för kunden.
...
sys.path.insert(0, BAS + "/runda-109")
...
RUNDAN = list(matt.RUNDAN)
RUBRIK = {k: "%s paneler i sicksack" % ORD[T.PRODUKTER_IN[k][0]] for k in RUNDAN}
...
RADER = {k: list(_RADER) for k in RUNDAN}

if __name__ == "__main__":
    kortdata = {k: (KICKER, RUBRIK[k], RADER[k]) for k in RUNDAN}
    produkter = [{"kort": k, "spec": specrader(k)} for k in RUNDAN]
    foton = {k: "%s/panelfoton/%s.jpg" % (HAR, k) for k in RUNDAN}

# -*- coding: utf-8 -*-
"""Runda 112 Steg 7 — bygger PATCH-anropen och facit för verifieringen.

☠️ ALLA NIO ÄR UTKAST. `visible` skickas uttryckligen på varje PATCH —
   runbookens regel är att varje PATCH ska BÄRA `visible`, och runda 109 mätte
   upp att den halvan som är lätt att missa är VÄRDET, inte fältet. Värdet
   läses ur Wix strax före skrivningen och prövas mot det LÄSTA värdet, inte
   mot en konstant: en annan session polerar samma katalog samtidigt, så
   `false` är en förväntan, inte ett faktum.

☠️ NAMNET PÅ `1b87909f` RÄTTAS I DEN HÄR PATCHEN. Leverantörens namn säger
   100 tum; duken är 120. Tre oberoende mätningar säger samma sak — brödtexten
   tre gånger, geometrin (263 × 148 cm ger 118,8 tum) och ritningens egen
   `120"`. Det är alltså inte en omformulering utan en rättelse, och den syns
   bara om man läser namnet FÖRE och EFTER.

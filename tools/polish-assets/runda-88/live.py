# -*- coding: utf-8 -*-
"""Runda 88, Steg 14 — läs de åtta publicerade sidorna som kund.
...
maxlasten aldrig lånas mellan modellerna (50 kg på A och B, 100 kg på I),
att familje- och vuxenpåståendet bara står på modell I som faktiskt bär
en vuxen, att inget ord om elsparkcykel eller hastighet finns någonstans,
att ingen standard eller certifiering påstås, att hjälmrådet FINNS och
aldrig kallas lagkrav, att två bromsar bara utlovas på modell I och att
korgen bara nämns där den sitter — samt att modell I ALDRIG påstår en
hjulstorlek i tum, eftersom källan inte anger någon.
...
for p in texter.PRODUKTER:
    ...
    m = lint.MODELL[k]
    # ☠️ Maxlasten får ALDRIG lånas mellan modellerna. Tyskan säljer modell A
    #    på "hela familjen … även föräldrar" tre rader ovanför "50 kg".
    ...
    if not re.search(r"%d\s*kg" % lint.MAXLAST[m], region):
        fel.append("%s: modellens egen maxlast %d kg står inte på LIVE-sidan"
                   % (k, lint.MAXLAST[m]))
    # ☠️ Familje-/vuxenpåståendet bara på modell I, som faktiskt bär en vuxen.
    if m not in lint.FAMILJ_TILLATEN:
        for mt in lint.FAMILJ_RE.finditer(region):
            fel.append("%s: familje-/vuxenpåstående på LIVE-sidan — %r"
                       % (k, mt.group(0)))
    ...
    # ☠️ Två bromsar bara på modell I; korgen bara där den sitter.
    if m in lint.EN_BROMS:
        for mt in lint.TVA_BROMSAR_RE.finditer(region):
            fel.append("%s: två bromsar utlovade på LIVE-sidan — %r" % (k, mt.group(0)))
    if m not in lint.KORG_MODELL:
        for mt in lint.KORG_RE.finditer(region):
            fel.append("%s: korg/skärm på LIVE-sidan — %r" % (k, mt.group(0)))
    # ☠️ Modell I påstår ALDRIG en hjulstorlek i tum — källan anger ingen.
    #    Måttritningen ger 139, 58, 90–96, 37 och 12,5 cm och inget mer.
    if m == "I":
        for mt in re.finditer(r"\d+\s*tum", region, re.I):
            fel.append("%s: ohärledd hjulstorlek på LIVE-sidan — %r" % (k, mt.group(0)))

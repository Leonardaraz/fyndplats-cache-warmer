# -*- coding: utf-8 -*-
"""Runda 80, Steg 14 — läs de åtta publicerade sidorna som kund.
...
början. Kontrollmätt mot rundans egen text får dessa INTE stå med:
  ...
  ord        varför
  ---------  ------------------------------------------
  rollen     "Rollen" ÄR svenska
  rader      svenskt ord
  räder      svenskt ord (härjningståg)
  rosa       vår egen text säger "rosa" om hjärtstolen
  beige      svenskt ord
  komfort    svenskt ord
  stoff      svenskt "stoff"
  masse      svenskt ordled
  polster    ligger nära svenskans "polstring"; `polsterung` är säkert
  gas        vår text säger "gaslyft" ÖVERALLT
  arm        vår text säger "armstöd"
  hell/grun/matt/samt
             rena svenska ord eller svenska ordinledningar

☠️ `wippfunktion` ÄR säkert trots att vår text säger *vippfunktion* — tyskan
   har W där svenskan har V. Samma sak med `ergonomisch` mot svenskans
   *ergonomisk*: mönstret kräver ett `c` där svenskan har `k`. Båda är
   KONTROLLMÄTTA mot rundans egen text, inte antagna: hela listan körs mot
   `texter.bygg()` innan den används skarpt, och gav noll träffar.
...
TYSKA_STOL = [
    "drehstuhl", "bürostuhl", "burostuhl", "schreibtischstuhl", "chefsessel",
    "drehsessel", "relaxsessel", "arbeitsstuhl", "computerstuhl",
    "gamingstuhl", "arbeitshocker", "rollhocker", "drehhocker",
    "höhenverstellbar", "hohenverstellbar", "höhenverstellung",
    "sitzhöhe", "sitzhohe", "sitzkissen", "nackenkissen", "kopfstütze",
    "kopfstutze", "lendenstütze", "lendenstutze", "armlehne", "armlehnen",
    "fußkreuz", "fusskreuz", "gaslift", "gasfeder", "gasdruckfeder",
    "wippfunktion", "wippmechanik", "schaukelfunktion", "stufenlos",
    "belastbar", "tragfähigkeit", "tragfahigkeit", "atmungsaktiv",
    "ergonomisch", "leinenoptik", "polsterung", "taschenfederkern",
    "laufrollen", "bremsrollen", "gummirollen", "metallfuß", "metallfuss",
    "bodenarten", "geeignet", "robuster", "modernes", "schlichtes",
    "schlafzimmer", "hinweis", "aufbau", "bequem", "hellgrau", "dunkelgrau",
    "schwarz", "weiß", "weiss", "grau", "creme",
]

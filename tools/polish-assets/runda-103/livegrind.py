...
KONTROLL = "massagefatolj-svart-vippfunktion-landvarme"   # syskonet, ska OCKSÅ vara ren
...
TYSKA = re.compile(r"(?<![A-Za-zÅÄÖåäö])(Massagesessel|Wärmefunktion|Kunstleder|"
                   r"Hellgrau|Dunkelbraun|Cremeweiß|Hellbraun|Schwarz|drehbar|Modi|Liege)"
                   r"(?![A-Za-zÅÄÖåäö])")
...
KONTROLL_SYSKON = ["ljusgrått", "mörkbrunt", "cremevitt"]
...
for slug, pid, s in SIDOR:
    ...
    for f in ["82 × 99 × 103 cm", "135 kg", "145°", "Ø 9 cm", "82 × 165 × 78 cm"]:
        if f not in txt: FEL.append("%s: saknar %r" % (slug, f))
    for f in ["85 × 94", "150 kg", "150°"]:
        if f in txt: FEL.append("%s: ☠️ bär a0760ed1:s tal %r" % (slug, f))

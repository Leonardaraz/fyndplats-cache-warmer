# -*- coding: utf-8 -*-
"""Runda 108 — sex rumsavdelare i polypropenväv på tallram, tre storlekar.

☠️ TEXTEN SKRIVS I FIL FÖRST (runda 64: 9 fel mot 0). En sträng som går rakt
   in i ett API-anrop kan inte grepas innan den lämnar chatten, och PATCH-svaret
   ekar tillbaka exakt det man skrev.

Varje tal kommer ur leverantörens egna spec-block, lästa i Wix 2026-09-09 och
låsta i `matt.py`. Två av dem rättas på vägen, båda importfel som står i alla
sex (se `STEG1.md`):

  • `Material: Kiefernholz` tappar polypropenet — väven ÄR varan.
  • `Mått: 320L x 1,6B x 170H` är tyskans B(reite)/T(iefe) omdöpta. 1,6 cm är
    panelens DJUP.

☠️ TRE GÅNGJÄRN PER SKARV är HÄRLETT, inte hittat på: 4 paneler har 3 skarvar
   och 9 gångjärn, 6 har 5 och 15, 8 har 7 och 21. Talet stämmer i alla tre
   storlekarna, och det är därför det får skrivas.

⚠️ VAD TEXTEN INTE FÅR LOVA. En fristående skärm står genom att vinklas — rakt
   utfälld faller den. Det finns ingen väggförankring och ingen medföljer. Väven
   är dessutom öppen: den skymmer insyn men mörklägger inte. Båda sakerna står
   i texten som egenskaper, inte som varningslista (Leonards regel 2026-08-14).
"""
...
KORT_FARG = {"vit": "vit", "brun": "brun", "natur": "natur"}


def slug_av(paneler, fargslug):
    """☠️ Kort med flit. SKU:n byggs ur sluggen och kapas vid 24 tecken från
    VÄNSTER, så en lång mitt hade ätit upp det som skiljer sidorna åt — fyra
    färgsyskon med samma SKU är runbookens dokumenterade fälla (batch 58)."""
    return f"rumsavdelare-{STORLEK[paneler][0]}-{fargslug}"


def syskon(nyckel):
    """Samma storlek i annan färg, och samma färg i annan storlek.

    Varje färg finns i exakt två av de tre storlekarna, så listan blir alltid
    två rader — ingen sida står ensam och ingen länkar till sig själv."""
    pan, _f, fargslug, _v, _pm = PRODUKTER_IN[nyckel]
    ut = []
    for k, (p2, f2, fs2, _v2, _pm2) in PRODUKTER_IN.items():
        if k == nyckel:
            continue
        if p2 == pan:
            ut.append((f"samma bredd i {KORT_FARG[fs2]}", slug_av(p2, fs2)))
        elif fs2 == fargslug:
            ut.append((f"samma {KORT_FARG[fs2]} i {STORLEK[p2][0]} cm", slug_av(p2, fs2)))
    return ut
    ...
    return dict(
        ...
        html=(
            ...
            + P(f"Skärmen väger {vikt} och fälls ihop till 40 × {hopfallt} × 170 cm. "
                ...
                f"rummet kräver.")
            + P("Finns också som "
                + " och ".join(L(s, t) for t, s in lankar) + ".")
...
if __name__ == "__main__":
    for k, d in PRODUKTER.items():
        ...
        print(f"  html  {len(d['html']):>4} tecken")

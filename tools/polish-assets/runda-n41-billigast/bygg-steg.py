#!/usr/bin/env python3
"""Runda N41: bygger steg 1:s spärrtillägg, steg3.js, steg4.js och steg5.js ur rundans filer.
...
if len(sys.argv) > 2 and sys.argv[1] == "--rattelse":
    ...
    ut = ["async function () {",
          "  // Genererad av runda N41:s bygg-steg.py --rattelse ur <kort>.html + raa-hash.tsv — skriv den aldrig för hand.",
          SUMMA_JS,
          "  const PLAN = ["]
    for p in plan:
        ut.append("    {")
...
if len(sys.argv) > 2 and sys.argv[1] == "--seo-rattelse":
    ...
    ut = ["async function () {",
          "  // Genererad av runda N41:s bygg-steg.py --seo-rattelse ur seo.tsv — skriv den aldrig för hand.",
          SUMMA_JS,
          "  const PLAN = ["]
    for r in plan:
...
if len(sys.argv) > 1:
    ...
    if bas.count(ankare) != 1:
        ...
        sys.exit(1)
    tillagg = (
        "  // Tillagd av runda N41:s bygg-steg.py: spärr även över namn, slug och SEO,\n"

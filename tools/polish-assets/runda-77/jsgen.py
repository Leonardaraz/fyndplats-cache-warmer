...
Runda 77. ☠️ Texten skrivs aldrig för hand i API-anropet (batch 64: nio fel inline mot
...
for(const p of P){
  ...
  // ⚠️ Runtimen svarar {data:{product}}; äldre rundor läste {product}.
  //    Formtolerant läsning i stället för att gissa: ett saknat revisionsnummer
  //    hade skickats som undefined och felet syns först i Wix svar.
  ...
  ut.push(p.k+"  ok rev="+rp.revision+" slug="+(typeof rp.slug==="string"?rp.slug:(rp.slug&&rp.slug.name))+" len="+L);
}
return ut.join("\n");
"""

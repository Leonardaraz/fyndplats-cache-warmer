# -*- coding: utf-8 -*-
"""Runda 76, Steg 14 — läs de åtta publicerade sidorna som kund.
...
☠️ ORDLISTAN ÄR VALD FÖR DEN HÄR FAMILJEN, inte ärvd. Två lärdomar gäller:

  * Ord som stavas likadant på svenska och tyska får ALDRIG stå i listan.
    ☠️ `rosa` är den farliga här — tre av rundans åtta stolar ÄR rosa, och
    ordet är lika svenskt som tyskt. Det är därför utelämnat medan `grau`,
    `braun`, `grün` och `weiß` står kvar: inget av dem inleder ett svenskt
    ord. Samma skäl håller `netz`-orden inne och `matt`/`samt` ute.
  * `Gelb` fällde en gång en korrekt sida för att det står inuti
    *re·gelb·undet*. Därför bär varje ord en ordgräns i BÖRJAN (`\\b`), som
    släpper igenom svenska ord med tysk delsträng men fångar böjda former.

  `ergonomisch` är säkert trots svenskans *ergonomisk*: efter `ergonomis`
  kräver mönstret ett `c`, och det svenska ordet har ett `k`. (Rundans egen
  lint förbjuder dessutom ordet på svenska — se `ERGONOMI_RE`.)
...
from lint import MASTE_STA                                           # noqa: E402
...
TYSKA_KONTORSSTOL = [
    ...
    "homeofficestuhl", "homeoffice-stuhl",
    "höhenverstellbar", "hohenverstellbar", "atmungsaktiv",
    "armlehne", "kippfunktion", "wippfunktion", "stoffbezug",
    "netzrücken", "netzrucken", "netzgewebe", "teddysamt",
    "ergonomisch", "bodenschonend", "mobilität", "mobilitat",
    "hellgrau", "dunkelgrau", "hellgrün", "hellgrun",
    "cremeweiß", "cremeweiss", "grau", "braun", "weiß", "grün",
]
